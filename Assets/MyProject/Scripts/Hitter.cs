...
public class Hitter : MonoBehaviour
{
    ...
    void Start()
	{
        spHint.sprite = GameManager.gameManager.sp[GameManager.gameManager.nextKind - 1];
        thissprenderer.enabled = true;

        SpriteRenderer spriteRenderer = GetComponent<SpriteRenderer>();
		if (spriteRenderer != null)
		{
			Color[] colorArray = new Color[] { Color.red, Color.cyan, Color.yellow, Color.green, Color.magenta };
            spriteRenderer.sprite = GameManager.gameManager.sp[GameManager.gameManager.currentKind - 1];
	...
	void OnTriggerEnter2D(Collider2D collider)
	{
		if (collider != null)
		{
            if (!m_hitflag && collider.tag == "bubble")
            {
                ...
                if (gridManager != null)
                {
                    GameObject newBubble = gridManager.Create(transform.position, GameManager.gameManager.currentKind);
                    GameManager.gameManager.GetNextKind();
                    if (newBubble != null)
                    {
                        GridMember gridMember = newBubble.GetComponent<GridMember>();
                        //Debug.Log("SAKGOINHOIANOGBADOKGMKL   " + transform.position);
                        if (gridMember != null)
        ...
        GameManager.gameManager.RefreshBubbleList();
    }
}
